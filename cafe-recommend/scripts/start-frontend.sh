...
ERROR_LOG_FILE="${LOG_DIR}/frontend-error-${TIMESTAMP}.log"
...
# Next.js 서버 시작
echo "포트 $PORT에서 Frontend 서버를 시작합니다. 로그 파일: ${LOG_FILE}, 에러 로그: ${ERROR_LOG_FILE}"
nohup npm run dev > "${LOG_FILE}" 2> "${ERROR_LOG_FILE}" &

echo "프론트엔드 서버가 백그라운드에서 시작되었습니다. PID: $!" 